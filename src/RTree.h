...
public:
	...
	/// Find k nearest neighbors respect to search rectangle
	double MinDist(Rect& a_rectA, Rect& a_rectB);	/// a_rectA is a point
	double MaxDist(Rect& a_rectA, Rect& a_rectB);	/// a_rectA is a point
	double Dist(Rect& a_rectA, Rect& a_rectB);	/// a_rectA and a_rectB are two points
	int KNearestNeighborsSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const int k, DATATYPE* a_resultlist, ELEMTYPEREAL* a_distlist, int& a_searchCount);
	void KNearestNeighborsSearchRec(Node* a_root, Rect& a_key, const int k, DATATYPE* a_resultlist, ELEMTYPEREAL* a_distlist, int& a_searchCount);
	...
	int CountNodes();
	void CountNodesRun(Node* a_root, int& count);
...
RTREE_TEMPLATE
	double RTREE_QUAL::MinDist(Rect& a_rectA, Rect& a_rectB)
{
	double dist = 0;
    ...
    {
        if (a_rectA.m_max[i] < a_rectB.m_min[i] && dist < a_rectB.m_min[i] - a_rectA.m_max[i])
            dist = a_rectB.m_min[i] - a_rectA.m_max[i];
        if (a_rectA.m_min[i] > a_rectB.m_max[i] && dist < a_rectA.m_min[i] - a_rectB.m_max[i])
            dist = a_rectA.m_min[i] - a_rectB.m_max[i];
    }
#endif
#ifdef MANHATTAN_DISTANCE
    for (int i = 0; i < NUMDIMS; ++i)
    {
        if (a_rectA.m_max[i] < a_rectB.m_min[i])
            dist += (a_rectB.m_min[i] - a_rectA.m_max[i]);
        if (a_rectA.m_min[i] > a_rectB.m_max[i])
            dist += (a_rectA.m_min[i] - a_rectB.m_max[i]);
    }
#endif
	...
	{
		if (a_rectA.m_max[i] < a_rectB.m_min[i])
			dist += (a_rectB.m_min[i] - a_rectA.m_max[i]) * (a_rectB.m_min[i] - a_rectA.m_max[i]);
		if (a_rectA.m_min[i] > a_rectB.m_max[i])
			dist += (a_rectA.m_min[i] - a_rectB.m_max[i]) * (a_rectA.m_min[i] - a_rectB.m_max[i]);
	}
    dist = sqrt(dist);
#endif
	return dist;
...
RTREE_TEMPLATE
	double RTREE_QUAL::MaxDist(Rect& a_rectA, Rect& a_rectB)
{
	double dist = 0;
#ifdef CHEBYSHEV_DISTANCE
    for (int i = 0; i < NUMDIMS; ++i)
    {
        if (dist < abs(a_rectB.m_max[i] - a_rectA.m_max[i]))
            dist = abs(a_rectB.m_max[i] - a_rectA.m_max[i]);
        if (dist < abs(a_rectB.m_min[i] - a_rectA.m_min[i]))
            dist = abs(a_rectB.m_min[i] - a_rectA.m_min[i]);
    }
#endif
#ifdef MANHATTAN_DISTANCE
    for (int i = 0; i < NUMDIMS; ++i)
    {
        if (a_rectA.m_max[i] < (a_rectB.m_min[i] + a_rectB.m_max[i]) / 2.0)
            dist += (a_rectB.m_max[i] - a_rectA.m_max[i]);
        else
            dist += (a_rectA.m_min[i] - a_rectB.m_min[i]);
    }
#endif
#ifdef EUCLID_DISTANCE
	for (int i = 0; i < NUMDIMS; ++i)
	{
		if (a_rectA.m_max[i] < (a_rectB.m_min[i] + a_rectB.m_max[i]) / 2.0)
			dist += (a_rectB.m_max[i] - a_rectA.m_max[i]) * (a_rectB.m_max[i] - a_rectA.m_max[i]);
		else
			dist += (a_rectA.m_min[i] - a_rectB.m_min[i]) * (a_rectA.m_min[i] - a_rectB.m_min[i]);
	}
    dist = sqrt(dist);
#endif
	return dist;
...
RTREE_TEMPLATE
	double RTREE_QUAL::Dist(Rect& a_rectA, Rect& a_rectB)
{
    double dist = 0;
#ifdef CHEBYSHEV_DISTANCE
    for (int i = 0; i < NUMDIMS; ++i)
...
RTREE_TEMPLATE
	int RTREE_QUAL::KNearestNeighborsSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const int k, DATATYPE* a_resultlist, ELEMTYPEREAL* a_distlist, int& a_searchCount)
{
#ifdef _DEBUG
	for(int index=0; index<NUMDIMS; ++index)
	{
	...
	{
		a_resultlist[i] = NULL;
		a_distlist[i] = DBL_MAX;
	}

	a_searchCount = 0;

	KNearestNeighborsSearchRec(m_root, key, k, a_resultlist, a_distlist, a_searchCount);

	int result = k;
	while (a_distlist[result - 1] == DBL_MAX)
		result--;
...
RTREE_TEMPLATE
	void RTREE_QUAL::KNearestNeighborsSearchRec(Node* a_root, Rect& a_key, const int k, DATATYPE* a_resultlist, ELEMTYPEREAL* a_distlist, int& a_searchCount)
{
	if (!a_root)
		return;

	if (a_root->IsInternalNode())
	{
		a_searchCount++;
		for (int index = 0; index < a_root->m_count; ++index)
		{
			if (MinDist(a_key, a_root->m_branch[index].m_rect) < a_distlist[k - 1])
				KNearestNeighborsSearchRec(a_root->m_branch[index].m_child, a_key, k, a_resultlist, a_distlist, a_searchCount);
		}
	}
	else
	{
		...
		for (int index = 0; index < a_root->m_count; ++index)
		{
			dis = Dist(a_key, a_root->m_branch[index].m_rect);
			if (dis < a_distlist[k - 1])
			{
				int i = k - 2;
				while (i >= 0 && dis < a_distlist[i])
				{
					a_distlist[i + 1] = a_distlist[i];
					a_resultlist[i + 1] = a_resultlist[i];
					i--;
				}
				a_distlist[i + 1] = dis;
				a_resultlist[i + 1] = a_root->m_branch[index].m_data;
			}
		}
	}
}
...
RTREE_TEMPLATE
	void RTREE_QUAL::CountNodesRun(Node* a_root, int& count)
{
	if (!a_root || a_root->IsLeaf())
		return;
	
	count++;
	if (a_root->IsInternalNode())
	{
		for (int i = 0; i < a_root->m_count; ++i)
			CountNodesRun(a_root->m_branch[i].m_child, count);
	}
}
